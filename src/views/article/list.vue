  ...
  <div class="app-container">
    <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%">
      ...
      <el-table-column width="180px" align="center" label="更新日期">
        <template slot-scope="scope">
          <span>{{ $dayjs(scope.row.updatedAt).format('YYYY-MM-DD HH:mm') }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column align="center" label="Actions" width="240">
        <template slot-scope="scope">
          ...
          <el-button style="margin-left:16px" type="danger" size="small" icon="el-icon-delete" @click="deleteArticle(scope.row.articleId)">
            Delete
          </el-button>
        </template>
      </el-table-column>
...
<script>
import { fetchArticle, fetchDeleteArticle } from '@/api/article'
...
export default {
  ...
  methods: {
    getList() {
      ...
      fetchArticle(this.listQuery.id).then(response => {
    ...
    deleteArticle(articleId) {
      fetchDeleteArticle({ articleId }).then(response => {
